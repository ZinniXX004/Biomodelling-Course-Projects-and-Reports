...
uses
  ...
  Vcl.Buttons, UnitForceGeneration, UnitHillTypeMuscleModel;
...
type
  TForm2 = class(TForm)
    ...
    ButtonForceGeneration: TButton;
    ButtonHillTypeMuscleModel: TButton;
    // *** TAMBAHKAN KOMPONEN SERIES BARU KE CHART2 DI FORM DESIGNER ***
    // 1. TLineSeries dengan nama: SeriesVerticalLine_FV
    SeriesVerticalLine_FV: TLineSeries;
    ...
    procedure ButtonForceGenerationClick(Sender: TObject);
    procedure ButtonHillTypeMuscleModelClick(Sender: TObject);
  private
...
begin
  ...
  SeriesVerticalLine.Pen.Color := clRed;
  SeriesVerticalLine.AddXY(lopt, Chart1.Axes.Left.Minimum);
  SeriesVerticalLine.AddXY(lopt, Chart1.Axes.Left.Maximum);
end;
...
begin
  ...
  // *** ADDED LINES ***
  // Menambahkan garis vertikal putus-putus pada velocity = 0
  // Pastikan SeriesVerticalLine_FV sudah ditambahkan ke Chart2
  SeriesVerticalLine_FV.Clear;
  SeriesVerticalLine_FV.Pen.Color := clBlack; // Warna merah
  SeriesVerticalLine_FV.Pen.Style := psDash; // Gaya putus-putus
  SeriesVerticalLine_FV.Title := 'Isometric';

  // Menambahkan dua titik untuk membuat garis vertikal di posisi velocity = 0
  SeriesVerticalLine_FV.AddXY(0, Chart2.Axes.Left.Minimum); // Titik bawah
  SeriesVerticalLine_FV.AddXY(0, Chart2.Axes.Left.Maximum); // Titik atas
  // *** END OF ADDED LINES ***
end;
...
begin
  ...
  // *** ADDED LINE ***
  // Membersihkan series tambahan di Chart2
  SeriesVerticalLine_FV.Clear;
  // *** END OF ADDED LINE ***
end;
...
procedure TForm2.ButtonHillTypeMuscleModelClick(Sender: TObject);
begin
  if Assigned(FormHillTypeMuscleModel) then
    FormHillTypeMuscleModel.Show;
end;

procedure TForm2.ButtonForceGenerationClick(Sender: TObject);
begin
  if Assigned(FormForceGeneration) then
    FormForceGeneration.Show;
end;
